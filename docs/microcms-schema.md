...
1. **API①「news」を管理画面から手作業で作成**(フィールド 4 個のみ。5〜10 分)
2. その API の **スキーマを JSON でエクスポート**して共有
   (API 設定 → スキーマのエクスポート)
3. その形式に合わせて **残り 3 API のスキーマ JSON をこちらで生成**
4. API 作成時に **インポート**して完了(45 項目の site-settings も手入力不要)

> 手順 2 のエクスポート JSON さえ頂ければ、以降の登録作業はほぼクリック操作だけになる。
...
> そのまま維持するため。並び順は登録順(または管理画面での並べ替え)で制御する。
...
| `awardWinners` | 受賞作品 | 繰り返しフィールド | | 下記のカスタムフィールドを繰り返す |

### 繰り返しフィールド `awardWinners` の中身(カスタムフィールド)
