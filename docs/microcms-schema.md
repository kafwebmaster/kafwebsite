...
| 方法 | 対象 | 手間 |
|---|---|---|
| a. 管理画面から手入力 | 全 API | site-settings 45 項目 + 画像 8 枚 + news 7 件 + 協賛 34 件で計 1〜2 時間 |
| b. **一時的な書き込み API キーで自動投入(推奨)** | テキスト・数値の値 | 書き込み権限キーを一時発行 → Claude がスクリプトで一括投入 → **投入後すぐキーを削除**。人間は画像 8 枚のアップロードのみ(15 分程度) |
| c. CSV インポート | リスト形式(news / sponsors) | 管理画面のコンテンツ CSV インポート機能を利用(プランの件数制限内) |

> 方法 b を採る場合も、キーの値はチャットに貼らず、投入作業の直前に発行し完了後に削除する。
